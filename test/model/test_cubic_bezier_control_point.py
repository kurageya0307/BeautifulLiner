...
class TestCubicBezierControlPoint(unittest.TestCase):
    ...
    def test_raise_error_with_set_p0_to_p3_as_int(self):
        ...
        #end with
        self.assertEqual(e.exception.args[0], 'p0 must be Point2D')

        with self.assertRaises(ValueError) as e:
            ctl_p = CubicBezierControlPoint(p, 0, p, p)
        #end with
        self.assertEqual(e.exception.args[0], 'p1 must be Point2D')

        with self.assertRaises(ValueError) as e:
            ctl_p = CubicBezierControlPoint(p, p, 0, p)
        #end with
        self.assertEqual(e.exception.args[0], 'p2 must be Point2D')

        with self.assertRaises(ValueError) as e:
            ctl_p = CubicBezierControlPoint(p, p, p, 0)
        #end with
        self.assertEqual(e.exception.args[0], 'p3 must be Point2D')
    #end
